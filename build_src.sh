...
if [ -z "$unregistry_VERSION" ] || [ -z "$BUILD_VERSION" ]; then
    echo "Usage: $0 <unregistry_version> <build_version>"
    echo "Example: $0 0.4.1 1"
    exit 1
fi

PACKAGE_NAME="unregistry"
ORIG_TARBALL="${PACKAGE_NAME}_${unregistry_VERSION}.orig.tar.gz"
BUILD_DIR="${PACKAGE_NAME}-${unregistry_VERSION}"

echo "Creating Debian/Ubuntu source packages for unregistry ${unregistry_VERSION}-${BUILD_VERSION}..."
...
if [ ! -f "$ORIG_TARBALL" ]; then
    ...
    wget -q "https://github.com/psviderski/unregistry/archive/refs/tags/${unregistry_VERSION}.tar.gz" -O "$ORIG_TARBALL"
...
build_source_package() {
    local dist=$1
    local FULL_VERSION="${unregistry_VERSION}-${BUILD_VERSION}~${dist}"
    ...
    rm -rf "$BUILD_DIR"
    tar -xf "$ORIG_TARBALL"
    # GitHub archives extract as unregistry-0.x.y/ which matches our BUILD_DIR
...
unregistry (${FULL_VERSION}) ${dist}; urgency=medium

  * New upstream release ${unregistry_VERSION}.
